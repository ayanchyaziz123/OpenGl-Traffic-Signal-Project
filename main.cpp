...
void drawstring(float x, float y, float z, char *string)
{
    ...
    for (c = string; *c != '\0'; c++)
    {
        ...
        glutBitmapCharacter(currentfont, *c); // render the characters
    }
}
...
void frontscreen(void)
{
    ...
    glColor3f(1, 0, 0);
    drawstring(450.0, 700.0, 0.0, "Metropolitan University, Bangladesh ");
    glColor3f(0.7, 0, 1);
    ...
    glColor3f(1, 1, 1);
    drawstring(100, 300, 0.0, "Name : Azizur Rahman Ayan");
    glColor3f(1, 1, 1);
    drawstring(100, 240, 0.0, "ID : 181-115-086");
    glColor3f(1, 0.5, 0);
...
void display(void)
{
    ...
    glClear(GL_COLOR_BUFFER_BIT);      // sets the bitplane area of the window to values previously selected by glClearColor, glClearDepth, and glClearStencil
...
void road()
{
    ...
    glScaled(40.0, 40.0, 0.0); // change the size of road up
    glColor3f(0.1, 0.1, 0.1);  // initialize with 3 color rgb
    glBegin(GL_POLYGON);       // making a polygon of car
    // straight road
    glVertex2f(0, 5); // specifies the x and y coordinates of the vertex, and the z coordinate is set to zero
    glVertex2f(40, 5);
    glVertex2f(40, 10);
    glVertex2f(0, 10);
    glEnd();
...
{
    glPushMatrix();            // making color for outer line
    glScaled(40.0, 40.0, 0.0); // change the size of signal up
    // stand
    glColor3f(0.1, 0.2, 0.1); // specifies the x and y coordinates of the vertex, and the z coordinate is set to zero
    glBegin(GL_POLYGON);      // making a polygon of stand
    glVertex2f(15, 7);
    glVertex2f(15, 8);
    glVertex2f(18, 8);
    glVertex2f(18, 7);
...
{
    glPushMatrix();              // making color for outer line
    glTranslated(b, 190.0, 0.0); // change the location of car
    glScaled(20.0, 20.0, 0.0);   // change the size of car
    glColor3f(1.0, 0.0, 0.0);
    glBegin(GL_POLYGON);  // making a polygon of car
    glVertex2f(2.5, 2.5); // specifies the x and y coordinates of the vertex, and the z coordinate is set to zero
    glVertex2f(3.0, 3.5);
    glVertex2f(3.5, 3.75);
    glVertex2f(4.0, 4.0);
    glVertex2f(4.5, 4.0);
...
void myinit()
{
    ...
    gluOrtho2D(0.0, 1346.0, 0.0, 728.0); // two-dimensional orthographic viewing region
}

int main(int argc, char *argv[])
{
    glutInit(&argc, argv);                       // glutInit will initialize the GLUT library and negotiate a session with the window system
    glutInitDisplayMode(GLUT_DOUBLE | GLUT_RGB); // creating top-level windows, subwindows, and overlays.
    glutInitWindowSize(1346, 728);
    glutInitWindowPosition(0, 0);
    glutCreateWindow("Traffic signal Project");
